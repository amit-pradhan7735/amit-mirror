...
def start(update, context):
    ...
    if EMOJI_THEME is True:
        ...
        buttons.buildbutton("✔️ Bot Owner ★", "https://t.me/woodcraft5")
    else:
        buttons.buildbutton(f"{START_BTN1_NAME}", f"{START_BTN1_URL}")
        buttons.buildbutton(f"{START_BTN2_NAME}", f"{START_BTN2_URL}")
        buttons.buildbutton("✔️ Bot Owner ★", "https://t.me/woodcraft5")
...
'''
        ...
        text = f"✔️Not Authorized user, deploy your own mirror bot"
        if PICS:
...
def restart(update, context):
    ...
    if dynoRestart:
        ...
        restart_message = sendMessage("✔️Sexy Bot Restarting•••", context.bot, update.message)
        if Interval:
            Interval[0].cancel()
            Interval.clear()
        alive.kill()
