...
def getleechinfo(from_user):
    ...
    if ospath.exists(thumbpath):
        thumbmsg = "Exists"
        buttons.sbutton("✔️Delete Thumbnail", f"leechset {user_id} thumb")
        buttons.sbutton("✔️Show Thumbnail", f"leechset {user_id} showthumb")
    else:
        thumbmsg = "Not Exists"
...
def setLeechType(update, context):
    ...
    if user_id != int(data[1]):
        ...
        path = f"Thumbnails/{user_id}.jpg"
        if ospath.lexists(path):
            msg = f"✔️Thumbnail for: {query.from_user.mention_html()} (<code>{str(user_id)}</code>)"
